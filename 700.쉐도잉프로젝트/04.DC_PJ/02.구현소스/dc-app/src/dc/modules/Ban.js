    ...
    $(() => {
        ...
        $(obj)
            ...
            .on("click", function () {
                ...
                $(obj).find(".indic li").eq(tg.find("li").eq(isB).attr("data-seq")).addClass("on").siblings().removeClass("on");
            }); ////////// click ////////////
        console.log("배너jqFn");
    }); ///////////// jQB ////////////////
} ///////////// jqFn 함수 /////////////////
...
function Ban(props) {
    ...
    const goSlide = (e) => {
        ...
        // 1. 버튼구분하기
        let isB = $(e.target).is(".rb");
        console.log("오른쪽?", isB);

        // 슬라이드 타겟설정 : 클릭된 버튼의 형제요소 슬라이더
        const tg = $(e.target).siblings(".slider");
        ...
        // isB값으로 오른쪽은 true->1, 왼쪽은 false->0
        // 순서가 바뀌는 슬라이드에 고유 순번속성 data-seq값을 읽어옴!
        $(e.target).parents(".banner").find(".indic li").eq(tg.find("li").eq(isB).attr("data-seq")).addClass("on").siblings().removeClass("on");
    };
    return (
        <div className="banner" onLoad={() => loadFn(this)}>
            {/* 이동 슬라이드 */}
            <ul className="slider">
                {sel_data.map((x, i) => (
